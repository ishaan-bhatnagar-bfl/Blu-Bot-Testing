// BLU Playwright Bridge Server v2.2
// Patches v2.1: retry card dismiss, input scroll-into-view, viewport bump
// Patches v2.2: verdict engine integration, double-bubble capture,
//               chip response tagged to test case, sourcing skip,
//               module passed through RUN_CASE/BULK_RUN
...
async function dismissRetryCard(maxWaitMs = 45000) {
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    if (!retryState.hasRetry) return true
    ...
    if (retryState.canClick) {
      ...
      if (retryState.countdown) {
        console.log(`⏳ Retry countdown: ${retryState.countdown}`)
        if (activeWs) activeWs.send(JSON.stringify({ type: 'RETRY_WAIT', countdown: retryState.countdown }))
      }
      await page.waitForTimeout(3000)
    }
  }
  console.log('⚠️  Retry card did not dismiss within timeout')
  return false
}
...
// FIX: waits for count AND text to fully stabilise across ALL new bubbles,
// not just the first one. Captures double-bubble responses correctly.
async function waitForBotToSettle(countBefore, maxMs = 30000) {
  const start = Date.now()

  // Phase 1: wait for at least one new bubble
  ...
  // Phase 2: wait for ALL bubbles to stop changing
  // Polls every 1s — if full DOM snapshot is same 4 times in a row, settled
  let prev = '', stable = 0
...
async function captureChatId() {
  try {
    ...
    console.log('ℹ️  Chat ID not captured')
  } catch (e) {
...
// FIX: captures ALL bubbles added since countBefore, merges into one response.
// This fixes the double-bubble issue where bot sends two messages rapidly.
async function getNewBotResponses(countBefore) {
  const start = Date.now()

  // Wait for loading phrases to clear on the LAST bubble
  const LOADING = ['hold on','please wait','checking','just a moment','fetching','kindly wait']
  let loadWait = 0
  ...
  if (loadWait > 0) console.log(`⏳ Waited ${loadWait}ms for loading`)

  try {
    const result = await page.evaluate((before) => {
      const allBotMsgs = Array.from(document.querySelectorAll('div.blu-bot-message'))

      // Collect all NEW bubbles (index > countBefore-1)
      const newBubbles = allBotMsgs.slice(before)
      if (!newBubbles.length) {
        // Fallback: just use last bubble
        newBubbles.push(allBotMsgs[allBotMsgs.length - 1])
      }

      // Merge all new bubble texts
    ...
    if (result.bubbleCount > 1) {
      console.log(`🤖 Captured ${result.bubbleCount} bubbles (${elapsed}s): ${result.text.substring(0, 100)}`)
    } else {
  ...
  return { response: '(response not captured)', bubbleCount: 0, chips: [], hasCTA: false, ctaLabels: [], elapsed: '?', isHinglish: false, chatId: null }
}
...
async function sendMessage(question, caseId = null, expectedBehaviour = '', module = '') {
  ...
  // FIX: pass countBefore so we capture ALL new bubbles
  const result  = await getNewBotResponses(countBefore)

  // ── VERDICT ENGINE (replaces autoScore) ───────────
  ...
  result.autoScore  = { score: verdict.verdict, confidence: verdict.confidence, reasons: verdict.rules.map(r => r.reason) }

  // Screenshot only on hard FAIL (not REVIEW or SOURCING_SKIP)
  let screenshot = null
...
// ── CLICK CHIP ─────────────────────────────────────
// FIX: tagged to caseId so dashboard associates chip response with correct test
...
// FIX: skips messages that are already being handled by sendMessage/clickChip
// by checking if a test is "in flight". Avoids double-logging.
...
async function startMessageObserver() {
  ...
  const interval = setInterval(async () => {
    if (!page || !activeWs || testInFlight) return
    try {
      ...
      if (result.count > lastCount && result.text !== lastText && result.text.length > 5) {
        ...
        if (!loading.some(p => result.text.toLowerCase().startsWith(p))) {
          ...
          logEntry({ type: 'passive', question: result.lastUser||'(chip click)', response: result.text })
        }
  ...
  wss.on('connection', ws => {
    ...
    ws.on('message', async raw => {
      ...
      if (msg.type === 'START_LOGIN') {
        ...
        testInFlight = true
        // FIX: module now passed from dashboard
        ...
        // FIX: pass caseId so chip response tagged to test
        testInFlight = true
    ...
    ws.on('close', () => { console.log('🔌 Dashboard disconnected'); testInFlight = false })
    ws.on('error', () => {})
  })
}
